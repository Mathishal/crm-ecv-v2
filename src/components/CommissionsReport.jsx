...
export default function CommissionsReport({ onOpenFacture }) {
  ...
  async function load() {
    ...
    const { data } = await supabase
      .from("commissions")
      .select("*, profiles(full_name,email), factures(id,number,total_ttc,issued_at)")
  ...
  const byCommercial = useMemo(() => {
    ...
    for (const c of commissions) {
      if (!groups.has(c.commercial_id)) {
        groups.set(c.commercial_id, { id: c.commercial_id, name: c.profiles?.full_name || "?", email: c.profiles?.email || "", items: [] });
      }
  ...
  async function markPaid(id) {
    await supabase.from("commissions").update({ paid: true, paid_at: new Date().toISOString().split("T")[0] }).eq("id", id);
    load();
  }

  async function markUnpaid(id) {
    await supabase.from("commissions").update({ paid: false, paid_at: null }).eq("id", id);
    load();
  }
              ...
              <div style={{marginTop:"12px"}}>
                {group.items.map(item => (
                  <div key={item.id} className="cl">
                    <div
                      style={{width:"20px",height:"20px",borderRadius:"5px",border:"2px solid var(--gm)",background:selectedIds.has(item.id) && !item.paid ? "var(--gm)" : "#fff",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,cursor:item.paid?"default":"pointer",opacity:item.paid?0.3:1}}
                      onClick={() => {
                        if (item.paid) return;
                    ...
                    >
                      {selectedIds.has(item.id) && !item.paid && <span style={{color:"#fff",fontSize:"12px",fontWeight:900}}>✓</span>}
                    </div>

                    <div className="cl__info" style={{cursor:onOpenFacture?"pointer":"default"}} onClick={() => onOpenFacture && item.factures && onOpenFacture(item.factures)}>
                      <div className="cl__num" style={{color:onOpenFacture?"var(--gm)":"var(--g9)"}}>{item.factures?.number}</div>
